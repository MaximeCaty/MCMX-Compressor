  ...
  public:
    ...
    static CMProfile CreateSimple(size_t inputs, size_t min_lzp_len = 10)
    {
      ...
      if (inputs > idx++)
        base.EnableModel(kModelOrder0);
      if (inputs > idx++)
        base.EnableModel(kModelOrder1);
      if (inputs > idx++)
        base.EnableModel(kModelOrder2);
      if (inputs > idx++)
        base.EnableModel(kModelOrder3);
      if (inputs > idx++)
        base.EnableModel(kModelOrder4);
      if (inputs > idx++)
        base.EnableModel(kModelOrder6);
      if (inputs > idx++)
        base.EnableModel(kModelOrder7);
      if (inputs > idx++)
        base.EnableModel(kModelOrder8);
      if (inputs > idx++)
        base.EnableModel(kModelOrder9);
  ...
  public:
    ...
    size_t ProcessBits(TStream &stream, const size_t c, size_t *base_contexts, size_t ctx_add)
    {
      ...
      do
      {
        ...
        uint8_t
            *rst sp0,
            *rst sp1, *rst sp2, *rst sp3, *rst sp4, *rst sp5, *rst sp6, *rst sp7,
        ...
        if (kInputs > 1)
          s1 = *(sp1 = &ht[base_contexts[1] ^ ctx_xor]);
        if (kInputs > 2)
          s2 = *(sp2 = &ht[base_contexts[2] ^ ctx_xor]);
        if (kInputs > 3)
          s3 = *(sp3 = &ht[base_contexts[3] ^ ctx_xor]);
        if (kInputs > 4)
          s4 = *(sp4 = &ht[base_contexts[4] ^ ctx_xor]);
        if (kInputs > 5)
          s5 = *(sp5 = &ht[base_contexts[5] ^ ctx_xor]);
        if (kInputs > 6)
          s6 = *(sp6 = &ht[base_contexts[6] ^ ctx_xor]);
        if (kInputs > 7)
          s7 = *(sp7 = &ht[base_contexts[7] ^ ctx_xor]);
        if (kInputs > 8)
          s8 = *(sp8 = &ht[base_contexts[8] ^ ctx_xor]);
        if (kInputs > 9)
          s9 = *(sp9 = &ht[base_contexts[9] ^ ctx_xor]);
        if (kInputs > 10)
          s10 = *(sp10 = &ht[base_contexts[10] ^ ctx_xor]);
        if (kInputs > 11)
          s11 = *(sp11 = &ht[base_contexts[11] ^ ctx_xor]);
        if (kInputs > 12)
          s12 = *(sp12 = &ht[base_contexts[12] ^ ctx_xor]);
        if (kInputs > 13)
          s13 = *(sp13 = &ht[base_contexts[13] ^ ctx_xor]);
        if (kInputs > 14)
          s14 = *(sp14 = &ht[base_contexts[14] ^ ctx_xor]);
        if (kInputs > 15)
          s15 = *(sp15 = &ht[base_contexts[15] ^ ctx_xor]);
        ...
        if (ret)
        {
          ...
          {
            if (kInputs > 0)
              *sp0 = NextState(sp0 - hash_table_, s0, bit, updater, 0, kOptP ? opts_[0] : 23);
          }
          if (kInputs > 1)
            *sp1 = NextState(sp1 - hash_table_, s1, bit, updater, 1, kOptP ? opts_[1] : 10);
          if (kInputs > 2)
            *sp2 = NextState(sp2 - hash_table_, s2, bit, updater, 2, kOptP ? opts_[2] : 9);
          if (kInputs > 3)
            *sp3 = NextState(sp3 - hash_table_, s3, bit, updater, 3, kOptP ? opts_[3] : 9);
          if (kInputs > 4)
            *sp4 = NextState(sp4 - hash_table_, s4, bit, updater, 4, kOptP ? opts_[4] : 9);
          if (kInputs > 5)
            *sp5 = NextState(sp5 - hash_table_, s5, bit, updater, 5, kOptP ? opts_[5] : 9);
          if (kInputs > 6)
            *sp6 = NextState(sp6 - hash_table_, s6, bit, updater, 6, kOptP ? opts_[6] : 9);
          if (kInputs > 7)
            *sp7 = NextState(sp7 - hash_table_, s7, bit, updater, 7, kOptP ? opts_[7] : 9);
          if (kInputs > 8)
            *sp8 = NextState(sp8 - hash_table_, s8, bit, updater, 8, kOptP ? opts_[8] : 9);
          if (kInputs > 9)
            *sp9 = NextState(sp9 - hash_table_, s9, bit, updater, 9, kOptP ? opts_[9] : 9);
          if (kInputs > 10)
            *sp10 = NextState(sp10 - hash_table_, s10, bit, updater, 10, kOptP ? opts_[10] : 9);
          if (kInputs > 11)
            *sp11 = NextState(sp11 - hash_table_, s11, bit, updater, 11, kOptP ? opts_[11] : 9);
          if (kInputs > 12)
            *sp12 = NextState(sp12 - hash_table_, s12, bit, updater, 12, kOptP ? opts_[12] : 9);
          if (kInputs > 13)
            *sp13 = NextState(sp13 - hash_table_, s13, bit, updater, 13, kOptP ? opts_[13] : 9);
          if (kInputs > 14)
            *sp14 = NextState(sp14 - hash_table_, s14, bit, updater, 14, kOptP ? opts_[14] : 9);
          if (kInputs > 15)
            *sp15 = NextState(sp15 - hash_table_, s15, bit, updater, 15, kOptP ? opts_[15] : 9);
        }
        if (kUseLZP)
        {
          if (kUseLZPSSE)
    ...
    size_t processByte(TStream &stream, uint32_t c = 0)
    {
      ...
      size_t n = (sse_ctx_ != 0) ? ProcessBits<decode, kBitTypeNormalSSE, kBitsPerByte>(stream, c, base_contexts, 0) : ProcessBits<decode, kBitTypeNormal, kBitsPerByte>(stream, c, base_contexts, 0);
      if (decode)
      {
        c = n;
      }
    ...
    static DataProfile profileForDetectorProfile(Detector::Profile profile)
    {
      ...
      case Detector::kProfileText:
        return kProfileText;
      case Detector::kProfileSimple:
        return kProfileSimple;
      }
