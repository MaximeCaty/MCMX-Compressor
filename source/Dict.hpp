...
public:
  ...
  public:
    void Generate(Builder &builder, CodeWordSet *words, size_t min_occurrences, size_t num_1 = 32, size_t num_2 = 32, size_t num_code_words = 128)
    {
      ...
      for (num3 = 0; num3 + num1 < num_code_words - num_2; ++num3)
      {
        ...
        if (count2 + count3 >= word_pairs.size())
          break;
      }
      words->num3_ = num3;
  ...
  public:
              ...
              for (size_t cur_len = word_len; cur_len >= min_len; --cur_len)
              {
                ...
                if (entry != nullptr)
                {
                  ...
                  *(out_ptr++) = code_word.byte1();
                  if (num_bytes > 1)
                    *(out_ptr++) = static_cast<uint8_t>(code_word.byte2());
                  if (num_bytes > 2)
                    *(out_ptr++) = static_cast<uint8_t>(code_word.byte3());
                  in_ptr += cur_len;
                  last_char_ = 'a';
                  next_word = true;
                  break;
    ...
    Filter(Stream *stream, size_t escape_char, size_t escape_cap_first = kInvalidChar,
        ...
        : ByteStreamFilter(stream), escape_char_(escape_char), escape_cap_first_(escape_cap_first), escape_cap_word_(escape_cap_word), last_char_(0)
    {
      init();
    }
    ~Filter()
    {
      ...
      {
        std::cout << std::endl
                  << "Escape " << escape_count_ << " word " << escape_count_word_ << " first " << escape_count_first_ << std::endl;
      }
    }
    void init()
    {
