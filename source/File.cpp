...
std::string FileInfo::attrToStr(uint16_t attr)
{
  ...
  if ((attr & kAttrDirectory) != 0)
    oss << 'd';
  if ((attr & kAttrReadPermission) != 0)
    oss << 'r';
  if ((attr & kAttrWritePermission) != 0)
    oss << 'w';
  if ((attr & kAttrExecutePermission) != 0)
    oss << 'x';
  if ((attr & kAttrSystem) != 0)
    oss << 's';
  if ((attr & kAttrHidden) != 0)
    oss << 'h';
  return oss.str();
}
