...
public:
  ...
  int usage(const std::string &name)
  {
    ...
    std::cout
        ...
        << "-{z|t|f|m|h|x}{1 .. 11} compression option" << std::endl
        << "z is super-turbo, t is turbo, f is fast, m is mid, h is high, x is max (default " << CompressionOptions::kDefaultLevel << ")" << std::endl
  ...
  int parse(int argc, char *argv[])
  {
    ...
    for (; i < argc; ++i)
    {
      ...
      if (arg == "-test")
        parsed_mode = kModeSingleTest; // kModeTest;
      else if (arg == "-memtest")
        parsed_mode = kModeMemTest;
      else if (arg == "-opt")
        parsed_mode = kModeOpt;
      else if (arg == "-stest")
        parsed_mode = kModeSingleTest;
      else if (arg == "c")
        parsed_mode = kModeCompress;
      else if (arg == "l")
        parsed_mode = kModeList;
      else if (arg == "d")
        parsed_mode = kModeDecompress;
      else if (arg == "a")
        parsed_mode = kModeAdd;
      else if (arg == "e")
        parsed_mode = kModeExtract;
      else if (arg == "x")
        parsed_mode = kModeExtractAll;
      if (parsed_mode != kModeUnknown)
      {
        ...
        opt_mode = true;
      else if (arg == "-filter=none")
        options_.filter_type_ = kFilterTypeNone;
      else if (arg == "-filter=dict")
        options_.filter_type_ = kFilterTypeDict;
      else if (arg == "-filter=x86")
        options_.filter_type_ = kFilterTypeX86;
      else if (arg == "-filter=auto")
        options_.filter_type_ = kFilterTypeAuto;
      ...
      else if (arg == "-lzp=auto")
        options_.lzp_type_ = kLZPTypeAuto;
      else if (arg == "-lzp=true")
        options_.lzp_type_ = kLZPTypeEnable;
      else if (arg == "-lzp=false")
        options_.lzp_type_ = kLZPTypeDisable;
      else if (arg == "-b")
      {
        if (i + 1 >= argc)
        {
      ...
      else if ((arg == "-threads") || (arg == "-thread"))
      {
        if (i + 1 >= argc)
          return usage(program);
      ...
      {
        if (arg[1] == 'z')
          options_.comp_level_ = kCompLevelZip;
        else if (arg[1] == 't')
          options_.comp_level_ = kCompLevelTurbo;
        else if (arg[1] == 'f')
          options_.comp_level_ = kCompLevelFast;
        else if (arg[1] == 'm')
          options_.comp_level_ = kCompLevelMid;
        else if (arg[1] == 'h')
          options_.comp_level_ = kCompLevelHigh;
        else if (arg[1] == 'x')
          options_.comp_level_ = kCompLevelMax;
        else if (arg[1] == 's')
          options_.comp_level_ = kCompLevelSimple;
        else
        {
        ...
        if (mem_string == "0")
          options_.mem_usage_ = 0;
        else if (mem_string == "1")
          options_.mem_usage_ = 1;
        else if (mem_string == "2")
          options_.mem_usage_ = 2;
        else if (mem_string == "3")
          options_.mem_usage_ = 3;
        else if (mem_string == "4")
          options_.mem_usage_ = 4;
        else if (mem_string == "5")
          options_.mem_usage_ = 5;
        else if (mem_string == "6")
          options_.mem_usage_ = 6;
        else if (mem_string == "7")
          options_.mem_usage_ = 7;
        else if (mem_string == "8")
          options_.mem_usage_ = 8;
        else if (mem_string == "9")
          options_.mem_usage_ = 9;
...
// Auto-select mem level based on file size and available RAM.
// Target: CM RAM usage ~ 2.9x total input size.
// Also cap to available physical RAM (leaving 512MB headroom).
static size_t autoMemLevel(const std::vector<FileInfo> &files, size_t current_level)
{
  ...
  for (const auto &f : files)
  {
    ...
    {
      File fin;
      if (fin.open(f.getFullName(), std::ios_base::in | std::ios_base::binary) == 0)
      {
        total_bytes += fin.length();
      }
    }
  }
  if (total_bytes == 0)
  ...
  // CM RAM at mem_level (approximate, for kInputs=2 / -z level):
  // hash=(2MB<<ml) + match=(0.5MB<<ml)*4 + cyclic=(0.25MB<<ml) + mixer small
  // Total ~ 2.75MB << ml
  static const uint64_t MB = 1024 * 1024;
  auto cm_ram = [](size_t ml) -> uint64_t
  {
    return (uint64_t(2 * 1024 * 1024) << ml)       // hash table
           + (uint64_t(1024 * 1024 / 4) << ml) * 2 // match model
           + (uint64_t(1024 * 1024 / 4) << ml)     // cyclic buffer
           + (uint64_t(0x100) << (4 + ml)) * 16;   // mixer
  };

  // Target: CM RAM <= file_size * 2.9
  const double kTargetRatio = 2.9;
  size_t best = 0;
  for (size_t ml = 0; ml <= 11; ++ml)
  {
    if (cm_ram(ml) <= uint64_t(double(total_bytes) * kTargetRatio))
    {
      best = ml;
    }
    else
  ...
  // Also check available physical RAM (leave 512MB headroom)
...
#endif
  static const uint64_t kHeadroom = 512ULL * 1024 * 1024;
  if (avail_ram > kHeadroom)
  {
    const uint64_t usable = avail_ram - kHeadroom;
    while (best > 0 && cm_ram(best) > usable)
    {
      --best;
    }
  }

  std::cout << "Auto mem level: file_size=" << formatNumber(total_bytes)
            << " -> mem=" << best
            << " (CM RAM=" << formatNumber(cm_ram(best)) << ")" << std::endl;
  return best;
}
...
int main(int argc, char *argv[])
{
  ...
  case Options::kModeMemTest:
  {
    ...
    if (opt_mode)
    {
      ...
      for (uint32_t opt = 0;; ++opt)
      {
        ...
        opt_file << "opt " << opt << " = " << comp_size << std::endl
                 << std::flush;
  ...
  case Options::kModeTest:
  {
    ...
    else
    {
      ...
      if (!options.has_mem_arg)
      {
        options.options_.mem_usage_ = autoMemLevel(options.files, options.options_.mem_usage_);
      }

      std::cout << "Compressing to " << out_file << " mode=" << options.options_.comp_level_ << " mem=" << options.options_.mem_usage_ << std::endl;
      Archive archive(&fout, options.options_);
      archive.setNumThreads(options.num_threads);
      uint64_t in_bytes = archive.compress(options.files);
  ...
  case Options::kModeDecompress:
  {
    ...
    archive.decompress(options.files.empty() ? "" : options.files.back().getName());
    fin.close();
