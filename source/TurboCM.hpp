...
public:
  ...
  void init()
  {
    ...
    for (auto &pr : isse2)
      pr.init();
#endif
    isse3.resize(256 * 256);
    for (auto &pr : isse3)
      pr.init();
    isse4.resize(256 * 256);
    for (auto &pr : isse4)
      pr.init();
  ...
  uint32_t processByteInternal(TStream &stream, uint32_t c = 0, size_t sse_ctx = 0)
  {
    ...
    do
    {
      uint8_t
          *no_alias s0 = nullptr,
                    *no_alias s1 = nullptr, *no_alias s2 = nullptr, *no_alias s3 = nullptr,
...
#define USE_ISSE 0
      ...
      int p = pr0.getP();
      auto &is1 = isse1[(p / kDiv) * 256 + *s1];
      p = is1.getP();
      auto &is2 = isse2[(p / kDiv) * 256 + *s2];
      p = is2.getP();
      auto &is3 = isse3[(p / kDiv) * 256 + *s3];
      p = is3.getP();
      p = is3.getP();
      p += p == 0;
#else
