...
public:
  ...
  ALWAYS_INLINE static bool Detect(uint32_t last_word, Window<Subtype> &window, OffsetBlock *out)
  {
    ...
    uint32_t fpos = 0;
    const uint32_t chunk_size = window.template Read<kEndianLittle>(fpos, 4);
    fpos += 4;
    const uint32_t format = window.template Read<kEndianBig>(fpos, 4);
    fpos += 4;
    // Format subchunk.
    const uint32_t subchunk_id = window.template Read<kEndianBig>(fpos, 4);
    fpos += 4;
    ...
    const uint32_t subchunk_size = window.template Read<kEndianLittle>(fpos, 4);
    fpos += 4;
    if (subchunk_size != 16 && subchunk_size != 18)
    {
      return false;
    }
    const uint32_t audio_format = window.template Read<kEndianLittle>(fpos, 2);
    fpos += 2;
    const uint32_t num_channels = window.template Read<kEndianLittle>(fpos, 2);
    fpos += 2;
    if (audio_format == 1 && num_channels == 2)
    {
      ...
      const uint32_t bits_per_sample = window.template Read<kEndianLittle>(fpos, 2);
      fpos += 2;
      for (size_t i = 0; i < 5; ++i)
      {
        const uint32_t subchunk2_id = window.template Read<kEndianBig>(fpos, 4);
        fpos += 4;
        const uint32_t subchunk2_size = window.template Read<kEndianLittle>(fpos, 4);
        fpos += 4;
  ...
  void CompressBlock(SOut &sout, int16_t *samples, size_t num_samples, float *last_w, size_t iterations = 20)
  {
    for (size_t j = 0; j < 1; ++j)
      for (size_t i = num_samples - 1; i >= 2; --i)
    ...
    for (size_t i = 0; i < num_samples; ++i)
    {
      ...
      {
        if (use_gd)
          fpred += last[(last_pos - 1 - j) & kLastMask] * p.GetWeight(j);
        else
          fpred += last[(last_pos - 1 - j) & kLastMask] * -coeff[j];
      }
  ...
  virtual void compress(Stream *in_stream, Stream *out_stream, uint64_t max_count)
  {
      ...
      if (block_size >= max_block_size)
      {
        ...
        if (false && block_idx == 25)
        {
          ...
          for (size_t i = 0; i < block_size; ++i)
            vals.push_back(block[i] / 32767.0f);
          for (size_t j = 0; j < 2; ++j)
          {
            ...
            for (size_t i = j; i < vals.size(); i += 2)
              d1.push_back(vals[i] - vals[i - 2]);
            ...
            for (size_t i = 4 + j; i < vals.size(); i += 2)
              d3.push_back(vals[i] - (vals[i - 1] * last_w[0] + vals[i - 2] * last_w[1] + vals[i - 3] * last_w[2] + vals[i - 4] * last_w[3]));
    ...
    if (block_size > 0)
    {
      CompressBlock(sout, &block[0], block_size, last_w, 20);
      block_size = 0;
    }
    ent.flush(sout);
    sout.flush();
