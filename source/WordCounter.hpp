...
public:
  ...
  {
    std::cerr << std::endl
              << "Word counter used " << Used() << " hash size " << hash_mask_ << std::endl;
  }
  ...
  void Init(size_t memory)
  {
    ...
    while ((hash_mask_ & (hash_mask_ + 1)) != 0)
      --hash_mask_;
  ...
  void GC(size_t min_count)
  {
    ...
    std::cerr << std::endl
              << "GC " << prettySize(start_size) << " -> " << Used() << " in " << clockToSeconds(clock() - start) << "S" << std::endl;
  }
