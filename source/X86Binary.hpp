...
public:
  X86AdvancedFilter(Stream *stream)
      : ByteStreamFilter(stream), offset_(17), last_offset_(17), opt_var_(0), transform_count_(0), false_positives_(0)
  {
  }
  ...
  {
    std::cout << std::endl
              << "E8E9: " << transform_count_ << " / " << false_positives_ << std::endl;
  }
...
private:
  ...
  inline void handleE8E9(uint8_t *ptr, size_t cur_offset)
  {
    ...
    if (sign_byte == 0xFF || sign_byte == 0x00)
    {
      ++count_;
      uint32_t offset =
          encode ? static_cast<uint32_t>(ptr[1]) +
